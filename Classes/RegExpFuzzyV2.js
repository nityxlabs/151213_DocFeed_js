...
StrMetric.prototype = {
    ...
    getMultiIndex: function (re) {
        ...
        while ( match = re.exec(this.str) ) {
            // brackets.push( this.getBracket(match.index) );       //retrieve bracket of first position found
            brackets.push( this.getBracket(match.index + match[0].length - 1) );       //retrieve first position of string found
        }
    ...
    //inRange_backup is the same as inRange(), except it uses str.search(re), which finds first occurrence - good for single word matches
    inRange_backup: function (re, start, end) {
        //VAR: re = regular expression that will search in string. NOTE: need 'g' in regular expression
        //VAR: start = the starting bracket in this.brackets
        ...
        var nextIndex = searchStr.search(re);
        if (nextIndex == -1)            //nothing was found, return -1
            return nextIndex;
        else {
            var pos = this.brackets[start] + nextIndex;
            return this.getBracket(pos);
    ...
    inRange: function (re, start, end) {
        ...
        //extract string, find position, and then find new bracket
        var searchStr = this.extract(start, end);
        // var nextIndex = searchStr.search(re);        //faster, finds first match but doesn't consider multiword string length
        var nextIndex = re.exec(searchStr);     //slower, but considers multiword string length

        if (nextIndex == null)            //nothing was found, return -1
            return nextIndex;
        else {
            //add the bracket + the first position the string is found + the length of the string (length of string is useful for multiple word matches)
            var pos = this.brackets[start] + nextIndex.index + nextIndex[0].length - 1;     //perform '-1' because one of the characters belongs to 'nextIndex.index', & also to not jump brackets


            //TEST::
            // console.log( "inRange: re = ", re, " & start = ", start, " & end = ", end, " && searchStr = ", searchStr, " & nextIndex = ", nextIndex, " || & pos = ", pos, " & this.getBracket(pos) ", this.getBracket(pos) );
            // var pos2 = this.brackets[start] + nextIndex.index;
            // console.log( "pos --> BEFORE = ", pos2, " & posAFTER = ", pos, " & brackets ", this.brackets, " & re = ", re, " & searchStr = ", searchStr );
            // console.log( "this.getBracket: BEFORE = ", this.getBracket(pos2), " & AFTER = ", this.getBracket(pos) );
    ...
    extract: function (start, end) {
        ...
        if (start == null)
            start = 0;
        if ( (end == null) || (end > this.brackets.length) )
        ...
        //if start is not defined, do not return a string, else return the substring
...
function RegExpFuzzy(reZ) {
    ...
    //this phrase will be used to skip elements in RegExpFuzzy      
