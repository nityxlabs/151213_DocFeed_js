...
/*
REQUIRMENTS:
-needs DocManip.js file
*/

//Original common words list
...
var commonWordsText = "the,be,to,of,and,a,in,that,have,I,it,for,not,on,with,he,as,you,do,does,at,this,but,his,by,from,they,we,say,her,she,or,an,will,my,one,all,would,there,their,what,so,up,out,if,about,who,get,which,go,me,when,make,can,like,time,no,just,him,know,take,people,into,year,your,good,some,could,them,see,other,than,then,now,look,only,come,its,over,think,also,back,after,use,two,what,when,where,why,how,our,first,well,way,even,new,want,because,any,these,give,day,most,us,time,person,year,way,day,thing,man,world,life,hand,part,number,fact,be,have,do,say,said,get,make,go,know,take,see,come,think,look,want,give,use,find,tell,ask,seem,feel,try,leave,call,good,new,first,last,long,great,little,own,other,old,right,big,high,different,small,large,such,next,early,young,important,few,public,bad,same,able,was,were,is,are,has,have,it,those,had,did,run,ran,between,within,more,less,across,been,require,requires,however,where,whereas,though,although,furthermore,moreover,show,both,according,accordingly,begin,begun,determine,yet,many,often,may,might,et,al,must,during,meanwhile,while,upon";
...
classStemmer.prototype.translateWordToStem = function(arrSents, includeCW)
{
    //VAR: arrSents = array of sentences where each element is a sentence
    //VAR: includeCW = boolean that, if true, will include common words, else if false then will not include common words
    ...
    for(var s in arrSents)
    {
        ...
        for (var w in arrSW)
        {
            ...
            //else if word doesn't have stem & includeCW is true, then include common word, else excludes them
            else if ( includeCW )
                arrSS.push( arrSW[w] );
        }
...
classStemmer.prototype.docWordToStem = function(arrSents, includeCW) {
    //VAR: arrSents = array of sentences where each element is a sentence
    //VAR: includeCW = boolean that, if true, will include common words, else if false then will not include common words
    //METHOD: converts all words in a document into their corresponding stem words

    var arrStemSents = this.translateWordToStem(arrSents, includeCW);
    return arrStemSents.join(" ");
}

classStemmer.prototype.getBranchWords = function(word) {
    //METHOD: finds stem word to variable 'word', and then returns all words that have the same stem word
    //if word is not a key in hashOWS (Original Word to Stem), then return 
    if ( !(word in this.hashOWS) )
        return [];

    var stemWord = this.hashOWS[word];
    return this.hashSOW[stemWord];
}
